...
struct VulkanRenderer::Impl {
    ...
    //
    // A replay reuses the lighting blocks the frame was drawn with: each
    // environment and object block the frame wrote is kept once, and a draw
    // refers to them by index, so drawing the frame again rebuilds none of
    // them.
    ...
    struct RecordedDraw {
        ...
        VkPipeline pipeline{};
        bool textured{};
        std::uint64_t texture{};             // texture cache key, when textured from the cache
        VkDescriptorSet texture_descriptor{};  // what the draw sampled
        bool framebuffer_texture{};          // sampled a render target, which outlives the frame
        std::uint32_t environment{kNoBlock}; // index into FrameRecord::environments
        std::uint32_t object{kNoBlock};      // index into FrameRecord::objects, when lit
    ...
    struct FrameRecord {
        ...
        std::vector<EnvironmentBlock> environments;
        std::vector<ObjectBlock> objects;
        // Which of the renderer's block writes environments.back() and
        // objects.back() copy.
        std::uint64_t environment_write{};
        std::uint64_t object_write{};
        std::uint64_t texture_evictions{};  // the texture cache's evictions when recording began
        ...
        void clear() {
            ...
            environments.clear();
            objects.clear();
            environment_write = object_write = 0u;
            valid = false;
        }
    };
    ...
    // object blocks have been written: what a recording copies them by.
    EnvironmentBlock last_environment{};
    std::uint64_t environment_writes{};
    std::uint64_t object_writes{};
...
void VulkanRenderer::Impl::begin_recording() {
    ...
    if (writeback_in_flight) {
        writeback_pixels.resize(static_cast<std::size_t>(kPspWidth) * kPspHeight);
        std::memcpy(writeback_pixels.data(), writeback_mapped, writeback_pixels.size() * 4u);
        writeback_ready = writeback_recorded;
...
void VulkanRenderer::submit(const DrawCall &call, const GuestMemory &memory) {
    ...
    if ((lit || fogged) && impl.environment_version != call.environment_version) {
        ...
        impl.last_environment = environment;
        ++impl.environment_writes;
    }
    if (lit) {
        const LightingState &state = call.lighting;
    ...
    draw.framebuffer_texture = framebuffer_texture;
    draw.texture = draw.textured && !framebuffer_texture ? impl.last_texture_key : 0u;
    draw.texture_descriptor = texture_descriptor;
    // The blocks this draw reads, each kept once per frame.
    if (lit || fogged) {
        if (record.environments.empty() || record.environment_write != impl.environment_writes) {
            record.environments.push_back(impl.last_environment);
            record.environment_write = impl.environment_writes;
        }
        draw.environment = static_cast<std::uint32_t>(record.environments.size() - 1u);
    }
    if (lit) {
        if (record.objects.empty() || record.object_write != impl.object_writes) {
            record.objects.push_back(impl.last_object);
            record.object_write = impl.object_writes;
        }
        draw.object = static_cast<std::uint32_t>(record.objects.size() - 1u);
    }
...
void VulkanRenderer::Impl::finish_frame(std::uint32_t displayed, std::uint64_t virtual_us) {
    ...
    recording_frame.texture_evictions = texture_evictions;
...
void VulkanRenderer::Impl::replay(float t) {
    ...
    // The replay writes its own blocks into the vertex buffer and binds its
    // own state; the game's next draw writes and binds everything again.
    forget_bindings();
    environment_version = 0u;
    object_valid = false;
    // A texture dropped from the cache since the older frame was recorded may
    // be one it drew with: then every cached texture is looked up again.
    const bool descriptors_valid = texture_evictions == older.texture_evictions;
    std::uint32_t written_environment = kNoBlock;
    ...
    for (std::size_t i = 0; i < older.draws.size(); ++i) {
        ...
        if (partner >= 0 && t != 0.0f) {
            ...
            // A lit draw's object block carries its world matrix, which the
            // lights are evaluated in; everything else in it is kept.
            if (object != nullptr && world != object->world) {
                if (draw.object != written_object || world != written_world) {
                    blended_object = *object;
                    blended_object.world = world;
                }
                object = &blended_object;
            }
        ...
        // as the frame wrote them; a blended world makes a new object block.
        if (draw.environment != kNoBlock && draw.environment != written_environment) {
            if (!write_uniform(&older.environments[draw.environment], sizeof(EnvironmentBlock), environment_offset))
                break;
            written_environment = draw.environment;
        }
        if (object != nullptr && (draw.object != written_object || object->world != written_world)) {
            if (!write_uniform(object, sizeof(ObjectBlock), object_offset)) break;
        ...
        if (!lighting_bound || lighting_offsets != bound_lighting_offsets) {
            vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_layout, 1u, 1u,
                                    &lighting_descriptor, static_cast<std::uint32_t>(lighting_offsets.size()),
                                    lighting_offsets.data());
            bound_lighting_offsets = lighting_offsets;
            lighting_bound = true;
        }
...
bool VulkanRenderer::present(std::uint32_t display_address, std::uint64_t virtual_us) {
    ...
    if (!impl.interpolation_wanted() || impl.holding || source == VK_NULL_HANDLE || !impl.begin_cycle(source)) {
